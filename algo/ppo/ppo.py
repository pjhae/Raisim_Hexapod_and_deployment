...
class PPO:
    ...
    def log(self, variables, width=80, pad=28):
        ...
        self.writer.add_scalar('Info/Policy/mean_noise_std', mean_std.item(), variables['it'])

    # custom logger for rewards
    def log_reward(self, reward_names, rewards_dict, update):
        for name in reward_names:
            self.writer.add_scalar(f'Rewards/mean/{name}', np.mean(rewards_dict[name]), update)
            self.writer.add_scalar(f'Rewards/std/{name}', np.std(rewards_dict[name]), update)

    # custom logger for dones
    def log_info(self, dones, elapsed, update):
        self.writer.add_scalar(f'Info/Train/dones', dones, update)
        self.writer.add_scalar(f'Info/Train/elapsed', elapsed, update)
