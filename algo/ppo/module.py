...
class CNN(nn.Module):
    def __init__(self, shape, input_size, output_size):
        ...
        self.conv_leg_joints = weight_norm(nn.Conv1d(1, 16, kernel_size=3, stride=3, dilation=1))
        self.conv_legs = weight_norm(nn.Conv1d(16, 32, kernel_size=6, stride=6, dilation=1))
        self.conv_hist = weight_norm(nn.Conv1d(32, 64, kernel_size=3, stride=1, dilation=1))
        self.fc1 = weight_norm(nn.Linear(64+3, 128))
        self.fc2 = weight_norm(nn.Linear(128, output_size))
        self.activation = nn.ReLU()
    ...
    def forward(self, x):
        ...
        return self.fc2(self.fc1(torch.cat((y, x[:, -3:]), dim=1)))
